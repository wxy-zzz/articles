...
---
こんにちは。
株式会社IVRy（アイブリー）のデータアナリストの [wada](https://note.com/wxy_zzz/n/nd1e905d15842) です。
...
プロダクトには、自動対応できた着電件数や IVRy を使うことによって効率化できた時間などの統計情報を確認する機能があります。
...
今月、閲覧できる統計情報を大幅にアップデートしたダッシュボード機能がリリースされました。
IVR（音声自動応答）の各分岐の流入数や、長期の統計情報など、より詳細な情報が閲覧できるようになりました。
...
IVRy では解析用の各種データを BigQuery に集約しており、統計情報の元となる発着電イベントログも BigQuery に取り込んでいます。
BigQuery 上の発着電イベントログから統計データを計算するクエリを実行し、その結果をプロダクト側に転送するデータパイプラインを [TROCCO](https://trocco.io) を使って実現し、定期的にプロダクトにデータを転送しています。

![](/images/ivry-dbt-into-trocco-workflow-20241119/before.png)
*データの流れのイメージ（Before）*

既存の統計データ計算用クエリは直接 TROCCO に登録する形で実装していましたが、新しいダッシュボード機能の実現のために統計データ計算用クエリを大幅にアップデートする上で、

- クエリ自体の正しさのテスト
- stg/prd などの環境別で動作しているクエリのバージョン管理

が懸念点となりました。
GUI からの操作で起こり得るヒューマンエラーを低減することが課題です。

## dbt 導入後 - 改善されたこと

上記課題を解消するため、データモデリングの部分を dbt に寄せて「統計データモデルの作成」と「データの転送」部分を切り分けることで、集計結果の正しさを検証しやすい環境に変更しました。
...
プロダクトへのデータ転送パイプライン単体としてはシンプルに良くなったなという感覚なのですが、会社に横断的に dbt Core を導入するという視点で dbt project の管理体制をどうしていくかちょっと悩んでいます。

もともとは事業計画やプロダクト改善のための分析用のデータパイプラインを改善する目的で新規に dbt Core を導入し始めているため、現状は dbt というツールに詳しい人がまだ多くないため知見を集約しておく目的で一つの GitHub リポジトリに複数の dbt project を含めて管理しています。
...
上記のように、複数の dbt project をモノレポで管理している状態で、各 project の開発環境別のデプロイを実現したいときにどのようにするべきなのかに悩んでいます。

一旦 `main` ブランチは、全ての project の prd 環境リリース可能な状態のものが入っているとしているのですが、問題は各 project の検証状態のものをどう実現するかです。

TROCCO は指定したブランチの最新の状態の dbt を実行するので、各 project の stg 環境用のコードに対応するブランチを作成して TROCCO からの dbt 実行ができるようにする方針をとっているのですが、dbt project と同じ数の stg 環境用ブランチが増えてしまうので、今後管理が複雑になるのではないかと懸念しています。
...
```
今は dbt project の数が多くないため、この形式でも運用できそうではあるのですが、今後は IVRy では [複数のプロダクトを立ち上げていく](https://pm-notes.com/pdm-voice-57/) ことが予想されるため、負荷なくスケールする仕組みに移行していきたいところ……
