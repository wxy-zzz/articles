...
既存の統計データ計算用クエリは GUI で手動で直接 TROCCO に登録する形をとっていましたが、よりシステマチックな管理形態に移行することが一つの課題でした。
...
また、TROCCO のワークフローの中で dbt test を実行するようにすることで、クリティカルなエラーが起きていないかをプロダクトへの転送前に検査できるようになりました。
...
dbt は導入し始めたばかりで dbt というツール自体に詳しい人は社内にまだ多くないため、知見を集約しておく目的で一つの GitHub リポジトリで複数の dbt project をまとめて管理しています。
...
悩んでいるのは、上記のように複数の dbt project をモノレポで管理している時に、各 project の開発環境別のデプロイを実現したいときにどのようにすべきかという点です。

一旦、`main` ブランチは、全 project について prd 環境にリリース可能な状態のものが入っているとしているのですが、問題は各 project の検証状態のものをどう実現するかです。TROCCO は設定したブランチの最新の状態の dbt を実行するので、とりあえずは各 project に対して stg 環境用のコードに対応するブランチをそれぞれ作成して TROCCO から dbt 実行ができるようにしているのですが、この方針だと dbt project と同じ数の stg 環境用ブランチが増えてしまうので、今後管理が複雑になるのではないかと懸念しています。
